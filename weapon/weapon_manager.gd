...
func _ready():
	...
	get_parent().get_node("Camera/Aimcast").add_exception(owner) # Adds exception of player to the shooting raycast
	
	all_weapons = {
		"Unarmed" : preload("res://weapons/Unarmed/Blank.tscn"),
		"Durendahl" : preload("res://weapons/Armed/Durendahl.tscn")
	}
	...
	for w in weapons:
		if weapons[w] != null:
			...
			weapons[w].ray = get_parent().get_node("Camera/Aimcast")
