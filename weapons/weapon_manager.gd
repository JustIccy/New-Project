...
func _ready():
	...
	get_parent().get_node("Aimcast").add_exception(owner)
	
	all_weapons = {
		"Unarmed" : preload("res://weapons/Armed/Durendahl/Durendahl.tscn"),
		"Durendahl" : preload("res://weapons/Unarmed/Blank.tscn")
	}
	...
	for w in weapons:
		if weapons[w] != null:
			...
			weapons[w].ray = get_parent().get_node("Aimcast")
